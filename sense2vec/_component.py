...
class Sense2VecComponent(object):
    ...
    def __init__(self, vectors=None, freqs=None):
        if vectors is None:
            vectors = Vectors()
        if freqs is None:
            freqs = {}
    ...
    def s2v_freq(self, obj, attr='pos'):
        freq = self.freqs[self._get_query(obj, attr)]
        return freq

    def s2v_vec(self, obj, attr='pos'):
        vector = self.vectors[self._get_query(obj, attr)]
        return vector

    def s2v_most_sim(self, obj, n_similar=10, attr='pos'):
        vector = self.vectors[self._get_query(obj, attr)]
        words, scores = self.vectors.most_similar(vector, n_similar)
    ...
    def to_bytes(self, exclude=tuple(), **kwargs):
        """Serialize the sense2vec vectors to a bytestring.
        ...
        if self.vectors not in (True, False, None):
            serialize["vectors"] = self.vectors.to_bytes
        if self.freqs not in (True, False, None):
            serialize["freqs"] = lambda: srsly.msgpack_dumps(self.freqs)
    ...
    def from_bytes(self, bytes_data, exclude=tuple(), **kwargs):
        ...
        def load_vectors(b):
            self.vectors.from_bytes(b)

        def load_freqs(b):
            self.freqs = srsly.msgpack_loads(b)

        deserialize = OrderedDict()
        deserialize["cfg"] = lambda b: self.cfg.update(srsly.json_loads(b))
        deserialize["vectors"] = load_vectors
        deserialize["freqs"] = lambda b: load_freqs
        exclude = util.get_serialization_exclude(deserialize, exclude, kwargs)
        util.from_bytes(bytes_data, deserialize, exclude)
        return self

    def to_disk(self, path, exclude=tuple(), **kwargs):
        """Serialize the sense2vec vectors to disk."""
        serialize = OrderedDict()
        serialize["cfg"] = lambda p: srsly.write_json(p, self.cfg)
        if self.vectors not in (None, True, False):
            serialize["vectors"] = lambda p: p.open("wb").write(self.vectors.to_bytes())
        if self.freqs not in (None, True, False):
            serialize["freqs"] = lambda p: srsly.write_msgpack(p, self.freqs)
        exclude = util.get_serialization_exclude(serialize, exclude, kwargs)
    ...
    def from_disk(self, path, exclude=tuple(), **kwargs):
        ...
        def load_vectors(p):
            self.vectors.from_bytes(p.open("rb").read())

        def load_freqs(p):
            self.freqs = srsly.read_msgpack(p)

        deserialize = OrderedDict()
        deserialize["cfg"] = lambda p: self.cfg.update(_load_cfg(p))
        deserialize["vectors"] = load_vectors
        deserialize["freqs"] = load_freqs
        exclude = util.get_serialization_exclude(deserialize, exclude, kwargs)
